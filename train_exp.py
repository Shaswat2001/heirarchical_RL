...
def main(args):
    ...
    train_dataset = np.load(f"{args.dataset_dir}/go2_walk_maze.npy", allow_pickle=True).item()
    train_dataset["observations"] = np.array(train_dataset["observations"], dtype=np.float32)
    train_dataset["actions"] = np.array(train_dataset["actions"], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env_name', type=str, default='`go2-maze-play-v0', help='Environment (dataset) name.')
